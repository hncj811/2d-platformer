...
ExplodingSprite::ExplodingSprite(const std::string& s) :
  Sprite(s), 
  chunks(), 
  freeList(),
  frames() , 
...
{
  makeChunks(
    Gamedata::getInstance().getXmlInt(s+"/chunk/size")  
  );
}
...
void ExplodingSprite::update(Uint32 ticks) { 
  ...
  while (ptr != chunks.end()) {
    ptr->update(ticks);
    if (ptr->goneTooFar() || life > 5000 ) {  // Check to see if we should free a chunk
...
void ExplodingSprite::makeChunks(unsigned int n) { 
  ...
  if (speedx >= -10 && speedx <= 10) speedx = 25; // Make sure it's not 0;
  if (speedy == 0) speedy = 11; // Make sure it's not 0;
...
int ExplodingSprite::getLife() const 
{ return life; }
